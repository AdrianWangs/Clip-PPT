...
这里需要说明的是，PPT上的图2是简化示例，只展示了3个索引来帮助理解原理。在实际实现中，我们将128位哈希码平均分成64个独立段，每段2位，并为每段构建一个独立的倒排索引，总共形成64个倒排索引。
...
<!--
两层加密索引是本方案的核心创新。第一层使用对称可搜索加密(SSE)技术：对于LSH生成的64个倒排索引，我们使用64个独立的SSE密钥分别加密。用户查询时，为每个哈希段生成对应的陷阱门，云服务器可并行搜索这些加密索引，快速获得候选集。
...
### 索引构建阶段（数据所有者执行）
1. **特征提取**：使用CLIP模型提取所有图像的特征向量
2. **LSH哈希计算**：对每个特征向量生成128位哈希码，并分成64段
3. **倒排索引生成**：为每段哈希码构建一个独立的倒排索引
4. **SSE加密**：用64个独立密钥分别加密每个倒排索引（第一层）
5. **Secure kNN加密**：对原始特征向量进行加密（第二层）
6. **数据上传**：将加密的图像数据和两层加密索引上传至云端

### 查询检索阶段（用户执行）
1. **查询特征提取**：使用CLIP提取查询文本的特征向量
2. **查询哈希计算**：生成查询的128位哈希码并分成64段
3. **陷阱门生成**：生成SSE陷阱门和Secure kNN加密查询向量
4. **第一轮筛选**：云服务器并行搜索64个SSE索引，获得候选集
5. **候选集合并**：合并所有匹配的图像ID，形成第一轮结果
6. **第二轮精确排序**：在候选集上使用Secure kNN计算精确相似度
7. **结果返回**：返回Top-k个最相似的加密图像给用户
...
<!--
让我们通过一个完整示例来理解整个检索流程。在索引构建阶段，数据所有者首先使用CLIP模型在本地提取所有图像的特征向量，然后使用E2LSH算法生成哈希码并分成64段，为每段构建独立的倒排索引。接着，使用64个独立的SSE密钥对这些倒排索引进行加密（第一层索引），同时使用Secure kNN算法对原始特征向量进行加密（第二层索引）。最后，将所有加密后的索引和图像数据上传到云服务器。

在查询检索阶段，用户首先提取查询文本的CLIP特征，生成对应的哈希码并分段。然后，为每个哈希段生成SSE陷阱门，并为原始特征生成Secure kNN加密查询向量。云服务器接收到这些加密查询后，并行搜索64个SSE索引，获得第一轮候选结果。最后，使用Secure kNN算法计算候选结果与查询的精确相似度，返回Top-k个最相似的加密图像给用户。
