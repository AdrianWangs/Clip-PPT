...
综合两种情况，我们可以得出结论：加密后的特征向量和查询向量的内积等于它们的原始向量的内积。这个结论非常重要，它意味着我们可以在完全加密的状态下精确计算两个向量的语义相似度，而不会泄露任何原始数据信息。这真是一个既简单又巧妙的数学设计！
-->
...
<div class="bg-gray-50 p-4 rounded-lg">
  <h3 class="text-lg font-bold text-gray-800 mb-3">系统参与实体</h3>
  
  <h4 class="font-semibold text-gray-700 mt-4 mb-2">1. 数据所有者 (Data Owner, DO)</h4>
  ...
  <h4 class="font-semibold text-gray-700 mt-4 mb-2">2. 数据用户 (Data User, DU)</h4>
  <ul class="text-sm space-y-1">
    <li>从DO处获取密钥</li>
    <li>使用CLIP提取查询特征</li>
    <li>生成查询陷阱门</li>
  ...
  <h4 class="font-semibold text-gray-700 mt-4 mb-2">3. 云服务器 (Cloud Server, CS)</h4>
  <ul class="text-sm space-y-1">
    <li>存储加密的索引和数据</li>
    <li>接收用户的查询陷阱门</li>
    <li>执行加密检索操作</li>
...
<div class="mt-2">
  <img src="https://cdn-mineru.openxlab.org.cn/result/2025-09-20/14d62f6a-cd31-4ba1-8559-004c76699681/4b54b44966dbcab05f40169586f9e8c021d1bd9c36eab15cfea84a35a2224e82.jpg" alt="系统模型" class="w-full">
  <div class="text-xs text-gray-500 text-center mt-2">图1：系统模型示意图</div>
</div>
...
<div class="mt-6 p-4 bg-gray-50 rounded-lg">
  ...
  <div class="text-sm">
    ...
    <p class="mt-2"><strong>注意：</strong>根据Yao等人的研究，Secure kNN算法对选择明文攻击(chosen plaintext attack)不够安全，但可通过添加随机噪声维度提升安全性（代价是匹配准确性）。</p>
  </div>
</div>

<!--
本方案的系统模型包含三个主要实体：数据所有者(DO)、数据用户(DU)和云服务器(CS)。数据所有者负责整个系统的初始化，包括生成密钥、提取特征、构建索引和加密数据。数据用户从数据所有者处获取密钥，用于生成查询陷阱门和解密检索结果。云服务器提供存储和计算服务，但在整个过程中只能看到加密后的数据，无法获取任何明文信息。

在威胁模型方面，我们采用了广泛接受的"诚实但好奇"假设。这意味着云服务器会忠实地执行协议，但同时会尝试从接收到的数据中推断用户的隐私信息。我们假设数据所有者和数据用户是完全诚实的，不会与云服务器串通泄露密钥，并且他们之间的通信渠道是安全可靠的。在这种情况下，我们的攻击模型是密文攻击模型(COA)。
