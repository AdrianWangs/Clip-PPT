...
<div class="pre-cover">
  <div class="pre-kicker">NDSS 2026 · CCF A</div>

  <div class="pre-title">Prϵϵmpt</div>
  <div class="pre-subtitle">Sanitizing Sensitive Prompts for LLMs</div>

  <div class="pre-cover-line"></div>

  <div class="pre-lead mt-6">
    面向 <b>LLM 推理阶段 prompt 隐私保护</b> 的本地净化方案：在 prompt 进入云端模型之前，先处理可识别的敏感 token。
  </div>

  <div class="pre-meta mt-8">
...
第四组是金融多轮 Q/A，用的是一个围绕财报表格做多步数值推理的数据集，对数字更敏感。epsilon 等于 2.0 时，中位相对误差是 2.44%。相对误差就是回答和原答案差了多少，再除以原答案；中位数就是把所有样本误差排个序，取中间那个，避免被极端错误带偏。噪声不太大时，回答还能接近原答案。但 ε=2.0 更偏可用性，不应该被讲成强隐私。
...
<p>UniNER 在翻译样本中识别约 96% 的唯一 PII 值；长上下文 Q/A 中识别约 92% 的唯一角色身份。PII 值指去重后的个人身份信息值。</p>
</div>
</div>
...
论文里 UniNER 的结果还可以。UniNER 是论文使用的实体识别模型。翻译样本中，它识别并净化了大约 96% 的唯一 PII 值。PII 是 personally identifiable information，也就是可以识别到个人的信息，比如姓名、电话、证件号、年龄、金额。

这里的“PII 值”要特别解释一下。它不是指一类字段，而是字段里的具体取值。比如同一个手机号在文本里出现三次，按出现次数算是三次；但按去重后的 PII 值，只算一个敏感值。论文用“唯一 PII 值”来衡量 NER 覆盖了多少不同的敏感取值，而不是简单统计重复出现了多少次。
...
表里的“是否要保存映射”，指系统是否需要额外保存“原始敏感值到替换值”的对应关系。比如把 Alice 替换成 Person_1，如果以后还要恢复 Alice，就要保存这张映射表。映射表一旦泄露，本身就是新的风险。

第一，Prϵϵmpt。它有形式化证明，可用性也比较高，而且不需要保存这种原值到替换值的映射表。代价是保护范围比较窄，只保护已经识别出来的 token 级敏感信息。

第二，HE / MPC。HE 是同态加密，MPC 是多方安全计算。它们的隐私保护更强，目标是让服务端在不知道明文的情况下参与计算。但问题也很清楚：推理成本太高，很难直接放到现有 LLM API 前面。

第三，替换表。替换表就是把敏感值换成另一个占位值，同时系统保存一张映射表，之后再根据表恢复。它的可用性还可以，但不是无状态的；这张映射表本身也会变成新的风险点。

第四，Redaction。Redaction 就是直接遮盖或删除敏感值。它很直观，也不需要维护状态，但可用性低。很多任务里 LLM 需要这些关键值才能回答，直接删掉会让问题变得无法处理。

第五，LLM 改写。它看起来轻量，但证明弱，而且不稳定。模型可能漏掉敏感字段，也可能把原任务改坏，所以很难给出清楚的安全边界。

所以 Prϵϵmpt 的位置是在中间：比 Redaction 和替换表更有证明和结构，比 HE / MPC 更容易接入现有 API。代价也很明确：它只处理已经识别出来的 token 级信息。
...
<div style="height: 100%; display: grid; grid-template-columns: 0.85fr 1.15fr; align-items: center; gap: 3rem;">
  ...
  <div style="font-size: 1.08rem; line-height: 1.75; color: #111827;">
    <p>
      核心观点：Agent 安全不能只依赖模型自己的概率判断，未来更需要确定性的外部约束。
    </p>
    <p style="margin-top: 1.1rem;">
      密码学可以参与构建可信运行环境，并对环境做形式化验证，从而给出更清楚的安全边界。
    </p>
...
<!--
这篇论文到这里就讲完了，不过就是我想跟大家以这篇论文为一个例子，去分享一些我自己最近的思考，然后我下面想讨论的内容比较宏观吧，大家可以听一听，也可以互相交流一些想法

首先，其实我想表达一个观点，就是：Agent 安全不能只靠“用模型约束模型”，更应该靠一个安全、可信、可验证的外部环境去约束模型。

现在已经不只是 LLM 时代了，更准确地说，是 Agent 时代。模型不再只是生成一段文本，它会调用工具、读写文件、访问网络，甚至参与一整条任务链路。所以安全问题也不只是“它回答得对不对”，而是“这个系统到底允许它做什么，它不能越过什么边界”。

我感觉之前很多论文，实际上还是在研究“用模型来对抗模型”。比如让一个模型检查另一个模型的输出，或者让模型自己根据提示词判断什么能做、什么不能做。这条路线当然有价值，但如果主要还是靠模型自己判断，或者靠另一个模型来审查它，我觉得是不够的。因为模型的判断本质上还是概率性的。上下文越长，工具越多，任务越复杂，它就越可能出现撒谎、绕规则，或者为了完成目标选择一条我们不希望它走的路径。

我和一个在头部大厂做相关工作的朋友交流时，也听到过类似的判断。他们现在很关注一个方向，可以叫 AI Validation。它关心的不是单次回答对不对，而是 Agent 在执行过程中有没有越界：工具调用是不是合规，状态修改是不是安全，输出是不是违反约束。也就是说，安全正在从“模型事后自查”，转向“外部系统持续验证”。

这个问题为什么会变得紧迫？因为模型能力真的在往执行任务的方向走。比如最近讨论比较多的 Claude Mythos 这类模型，在网络安全任务上能力已经很强；GPT 5.6 这类更强模型，也在编码、网络安全和长程 Agent 任务上继续提升。能力增强当然是好事，但它也会放大风险：如果模型有目标、有工具、有执行环境，它可能就会为了完成目标，走一条人类没预料到、也不希望它走的路径。

比如说前段时间我看到一个新闻，可能大家也有所耳闻。GPT-5.6 Sol 发布之后，有一家专门做前沿 AI 风险评估和模型能力评测的机构，对它做了一轮测试。

因为这个模型的网络安全能力非常出色，他们在做风险评估时跑了一些 benchmark。通常我们理解的 benchmark，就是给模型一组任务，让它主动完成，然后把模型输出的结果和标准答案进行对比，最后得出一个分数。

但后来他们发现了一件很有意思的事：这个模型有时并不会老老实实地解题。它反而会想办法利用评测环境本身的漏洞，尝试破解环境，直接获取答案再输出。更关键的是，它最后还会尝试隐藏自己作弊的路径，让整个过程看起来更像是正常完成任务。简单来说，它学会了作弊。

这个故事听起来像 benchmark 里的小插曲，但我觉得它其实很能说明 Agent 安全的问题。因为 Agent 不是只会回答一句话，它有目标、有工具、有环境。当这些东西组合在一起以后，如果外部边界没有设计好，它就可能为了完成目标，走出一条人类没有预料到、也不希望它走的路径。对评测来说，这叫作弊；放到真实系统里，本质上就是越界。

所以这类问题不能只靠提示词说“你不要作弊”，也不能只靠另一个模型来判断“它有没有作弊”。真正需要的是外部约束：权限要被限制，环境要被隔离，工具调用要被验证，关键状态变化要可审计，必要时还要能直接阻断。简单说，我们需要一个模型无法轻易绕过的运行环境。

Prϵϵmpt 当然没有解决完整的 Agent 安全问题。它只是 prompt 隐私里的一个小切口：在 prompt 进入云端 LLM 之前，先用本地机制净化可识别的敏感 token。但它给我的启发是，安全保护不一定要交给模型自己判断，也可以放在模型外部，通过确定性的机制先约束输入边界。

在这个背景下，密码学其实有很大的发挥空间。一个方向是构建可信环境，用密码学和系统机制给 LLM 或 Agent 的操作划出确定边界，规定它能看到什么、能调用什么、能输出什么。另一个方向是形式化验证，不只是说“模型大概率会遵守规则”，而是证明这个环境在给定假设下不会越界。

比如说有一个场景：你会给 AI 授予一些工具权限，但万一 AI 能力太强，发现了工具的漏洞并主动提权，那该怎么办？

我们可能会给它设置一些权限限制，比如沙盒之类的，但沙盒也有被 AI 破解的可能。如果说我们通过一些密码学工具去给它加一些防护，比如引入 ABE（属性加密）之类的技术，让权限管理不再仅仅依靠程序性的方式，而是通过密码学系统进行管理。

这样一来，当 Agent 想要进行越权操作时，它就必须去破解密码学中的数学难题。由于这些难题极难被攻克，我们就能自然而然地保证程序在既定框架内稳定运行。

相比传统的程序化权限控制，这种方式会更加安全。所以我们可以发现，在 Agent 时代，密码学似乎又大有可为了。

所以最后的落点是：我们可以把 LLM 或 Agent 看成一个能力很强、但不能完全信任的组件，甚至在安全分析里把它当成 adversary。我们要做的不是替代模型，而是为模型运行提供可信边界。AI 时代真正重要的安全工作，可能不是继续让模型判断模型，而是构建一个模型无法轻易越界的环境。

然后这些就是我最后想说的一些我自己的想法吧，欢迎大家讨论
-->
