...
<!--
大家好，今天和大家分享一篇关于LLM推理阶段隐私保护的论文——Prϵϵmpt。

这篇论文要解决的问题很直接：我们在使用GPT-4o这类云端LLM时，prompt里的敏感信息（比如SSN、信用卡号、年龄、薪资）会直接暴露给模型提供商。Prϵϵmpt的方案是在发送前对prompt做净化处理，用格式保留加密和metric差分隐私两种手段，分别处理不同类型的敏感token。

论文的核心贡献是：这是第一个同时做到有形式化隐私保证、高实用性、无状态、且资源高效的prompt净化系统。
-->
...
一位医生用GPT-4o帮病人分析体检报告，prompt里有患者SSN、诊断结果、保险号——发给模型提供商后，对方理论上都能看到
...
<!--
先看背景。想象一个场景：一位医生用GPT-4o分析病人体检报告，prompt里有SSN、诊断结果、保险号。这些信息发送给模型提供商后，对方理论上全部可见。推理阶段的对手不是别人，就是模型提供商自己。

而且in-context learning加剧了这个问题——用户直接把包含敏感信息的示例塞进prompt，等于把训练阶段的隐私风险搬到了推理阶段。
...
- 同态加密和安全多方计算可以提供强隐私保证
- 但计算开销过大：BERT上单次推理需要 **16分钟**
- 对GPT-4o这种规模的模型基本不可行
...
- **文本级DP**：对整段文本加噪，噪声随文本长度增长，实用性差
- **嵌入级DP**：对嵌入向量加噪再解码回文本，可能改变token类型
- **Token级DP**：从"相似token"列表中采样，需要精心构建列表
- **改写式DP**：生成噪声化的改写版本，同样面临维度诅咒

<div class="mt-2 text-center text-sm">

现有方案在四个目标之间顾此失彼：形式化隐私保证、高实用性、无状态、资源高效。**但如果不是所有token都需要同一种保护呢？**
...
第一条路是密码学，同态加密和MPC。隐私保证没问题，但代价太高。State-of-the-art的方案在BERT上单次推理要16分钟——注意是BERT，不是GPT-4o。规模差了几个数量级，这条路在实用性上直接不通。

第二条路是替换和混淆，用本地LLM把敏感token换掉。问题是反向恢复时需要查找表，这就有状态了。而且LLM的概率性质决定了你没法对它做严格的隐私分析。

第三条路是各种DP方案。对整段文本加噪的方案面临维度诅咒——噪声随文本长度线性增长。对嵌入加噪再解码的方案可能改变token类型，违反了prompt净化器的基本要求。

所以核心矛盾是：形式化保证、高实用性、无状态、资源高效，现有方案最多满足其中两三个，没有一个能全部做到。

那突破口在哪？关键观察是：不是所有敏感token都需要同一种保护方式。有些token（比如SSN）只需要保留格式，有些token（比如年龄）需要保留近似数值。针对不同类型用不同工具——这就是Prϵϵmpt的核心思路。
-->
...
**1. 提出 Prompt Sanitizer 这一密码学启发的原语**

- 形式化定义了prompt净化器的四个组件：Setup、Type Annotator、Sanitization、Desanitization
- 给出了隐私和实用性的形式化度量

**2. 实现 Prϵϵmpt 系统**

- 第一个同时满足形式化隐私保证、高实用性、无状态、资源高效的prompt净化系统
- 对敏感token分两类处理：Category I 用FPE（格式保留加密），Category II 用mLDP（metric局部差分隐私）

**3. 实验验证**

- 翻译任务：BLEU（翻译质量评分）几乎不变
- RAG任务：准确率100%
- 长上下文Q/A：STS（语义相似度）0.934（GPT-4o），优于Papillon
- 金融多轮Q/A：隐私预算增大时误差持续下降

<!--
论文的贡献分三层。

第一层是理论层面，提出了prompt sanitizer这个原语。这个定义借鉴了密码学的思路，把净化过程分成四个组件，然后分别给出隐私和实用性的形式化定义。隐私用博弈模型衡量——对手观察到净化后的prompt后，能多大程度区分两个原始prompt。实用性用质量函数衡量——净化后的响应和原始响应的差距有多大。

第二层是系统层面，Prϵϵmpt是这个原语的具体实现。它的关键设计决策是把敏感token分成两类：第一类是LLM响应只依赖格式的token（比如SSN、信用卡号），用格式保留加密处理；第二类是LLM响应依赖具体数值的token（比如年龄、薪资），用metric差分隐私处理。

第三层是实验验证，在翻译、RAG、长上下文Q/A、金融多轮Q/A四类任务上评估。结果支持了设计决策的有效性。
-->
...
威胁模型 → 两个核心机制（FPE + mLDP） → 端到端流程

<!--
接下来进入系统设计部分。我们先看威胁模型和设计目标，然后分别讲两个核心机制——格式保留加密和metric差分隐私，最后走一遍端到端的净化流程。
-->
...
1. **形式化保证** — 可证明的隐私保证
2. **高实用性** — 净化后的响应"接近"原始响应
3. **无状态** — 不保留session间信息，符合GDPR删除权
4. **资源高效** — 不需要修改LLM API
...
<!--
先看威胁模型。Prϵϵmpt的部署场景很清楚：它跑在用户本地，对手是云端的LLM提供商。用户把prompt交给Prϵϵmpt做净化，拿到净化后的版本再发给LLM，收到响应后再反净化。整个过程LLM只能看到净化后的内容。

保护范围上有个重要限制：Prϵϵmpt只保护从单个token就能提取的敏感信息。比如SSN、信用卡号、年龄这些——对手看到一个token就能直接利用，不需要理解上下文。至于"这段话暗示用户有抑郁症"这类需要语义推理的隐私问题，不在本文范围内。这个限制是合理的，因为独立token的隐私风险是最直接、最容易被利用的。

设计目标有四个，其中"无状态"值得展开说。无状态意味着Prϵϵmpt在session结束后不保留任何信息。这不只是技术选择，更是法律要求——GDPR和CCPA都规定了用户的数据删除权。有状态的方案需要维护映射表，这些表本身就是隐私风险。无状态设计直接消除了这个问题。
-->
...
第一步，用户做一次性注册，生成密钥和配置参数。第二步，提交prompt给Sanitizer，得到净化版本。第三步，把净化后的prompt发给LLM。第四步，LLM返回响应。第五步，用Desanitizer反净化响应。

注意信任边界的位置——红色虚线左边是可信区域，右边的LLM是不可信的。Prϵϵmpt的所有操作都在左边完成，LLM只能看到净化后的内容。

三个模块的分工也很清晰：Configuration Manager负责初始化，Sanitizer负责正向净化，Desanitizer负责反向恢复。关键是Desanitizer只需要密钥就能工作，不需要任何之前的session状态。
-->
...
Category I是"格式决定一切"的token。拿翻译任务举例：你让LLM把"My SSN is 055-46-6168"翻译成德语，LLM怎么处理SSN？它只需要知道这是一个SSN格式的字符串，然后原样保留。具体是哪9个数字，对翻译结果完全没有影响。所以这类token用格式保留加密就够了——密文和明文格式相同，LLM处理方式不变，解密后完美恢复。
...
<!--
FPE的核心思想可以一句话概括：在同格式的值空间上做置换。

拿SSN举例，所有9位数字的集合是一个有限集。FPE在这个集合上定义了一个由密钥控制的双射——每个合法SSN映射到另一个合法SSN。没有密钥的人看到密文569-83-4469，无法判断它是加密产生的还是一个真实的SSN，因为两者格式完全相同。

这个性质对Prϵϵmpt至关重要。LLM在处理净化后的prompt时，看到的仍然是格式正确的SSN、信用卡号、姓名，它的处理逻辑不会改变。翻译时SSN原样保留，RAG检索时信用卡号正常匹配，反净化时用密钥解密就能完美恢复。

而且FPE是确定性的——同一个密钥下，同一个明文总是产生同一个密文。这意味着prompt中如果同一个SSN出现多次，净化后仍然是同一个值，保持了一致性。

补充一个技术细节：FPE的安全强度依赖于格式空间的大小。SSN有10^9种可能值，安全性很好；但如果格式空间太小（比如性别只有M/F两个值），FPE的保护就很弱——因为随机置换的空间本身就很小。
-->
...
<!--
mLDP是Prϵϵmpt处理Category II token的工具。它和标准差分隐私的关键区别在一个词：距离感知。

标准LDP要求：无论输入是什么，输出分布都差不多。这意味着年龄46和年龄90在对手眼里应该一样难区分——这个保护强度对我们的场景来说太强了，代价是实用性会很差。
...
mLDP的数学含义可以这么理解：$e^{\epsilon \cdot d(x,x')}$ 是允许的概率比上界。距离越大，上界越宽松，对手越容易区分。距离为0时，上界是1，即完美不可区分。
-->
...
**两个性质的意义**：净化后的值大概率就是原值或其邻域，偏差可控——这直接保证了Prϵϵmpt的实用性

<!--
mLDP机制有两个性质直接决定了Prϵϵmpt的实用性。

Property 1说的是：净化后的值最可能就是原始值本身。这意味着大多数时候，LLM看到的数值和真实值相同或非常接近。

Property 2更进一步：如果净化值不是原始值，那么离原始值越近的候选值被选中的概率越高。两个性质合在一起保证了净化值的偏离是有限的、可控的。

然后是预算分配。如果prompt里有3个Category II token，总预算 $\epsilon$ 就平均分成 $\epsilon/3$，每个token各用一份。通过组合定理，总隐私损失不超过 $\epsilon$。
...
### 处理步骤（本文使用微调的 UniNER-7B-PII 做 NER）

1. **类型标注**：用NER模型 $\mathcal{M}_\tau$ 对prompt做命名实体识别，标注每个token的类型（Name、Age、SSN等）和类别（Category I / II）
...
第一步，NER标注。这里论文用的不是通用NER，而是专门针对隐私敏感实体微调过的UniNER。标注的输出不仅有类型（这是一个Age），还有类别（这个Age属于Category II）。
...
- 内容一致、隐私预算不增加（后处理免疫性——推导只看输出不看原始数据，所以不泄露额外信息）
...
$\Psi$ 解决这个问题的方式是：把依赖关系建模成有向无环图。根节点（独立变量）直接加噪，依赖节点通过编码的函数关系推导。这个推导属于后处理，由mLDP的后处理免疫性保证，不消耗额外隐私预算。
...
<!--
接下来看理论保证。这部分回答两个问题：第一，净化后的prompt到底泄露了多少？第二，净化后的响应质量能维持吗？
-->
...
- 对手选择两个相同泄露的prompt $(\rho_0, \rho_1)$
- 系统随机选一个净化后返回
- 对手猜测返回的是哪个 → 优势定义为 $\mathrm{Adv} = 2\Pr[\text{guess correct}] - 1$
...
对手的能力是：选择两个prompt，但这两个prompt必须有相同的"泄露"。所谓泄露函数 $\mathcal{L}$，指的是净化过程不可避免会暴露的信息——具体到Prϵϵmpt，就是所有非敏感token、Category I token的格式、以及Category II token的类型和数量。要求两个prompt在泄露函数下相同，就是排除对手通过观察"结构差异"来轻松区分的trivial情况。系统随机选一个净化后返回，对手猜是哪个。如果对手的优势（猜对概率减去1/2再乘2）可以被控制，就说明净化是有效的。
...
对于LLM的响应不依赖敏感token具体值的prompt，Prϵϵmpt提供**完美实用性**

$$\alpha = \mathbb{E}_f[Q(\rho, f(\rho))] = \mathbb{E}_{f,PS}[Q(\rho, D(K, f(\hat{\rho})))] = \beta$$

- 典型场景：翻译（token类型全是 $\tau_I$，FPE完美恢复）
- RAG中的格式查询也属于此类

### 非不变性 Prompts

**Lipschitz条件**：如果LLM在扰动输入下的响应变化有界

$$d_V(f(\rho), f(\hat{\rho})) \leq K \cdot d_V(\rho, \hat{\rho})$$

那么实用性损失和净化引入的扰动成正比——mLDP的小扰动特性保证损失可控
...
第一类是invariant prompt——LLM的响应对敏感token的具体值不敏感。翻译就是典型例子：你把SSN从055换成569，翻译结果不会有任何变化。对这类prompt，Prϵϵmpt的实用性是完美的：$\alpha = \beta$，响应质量完全不下降。

第二类是非invariant prompt——LLM需要用到敏感token的具体值。这时候需要额外条件。

如果prompt满足Lipschitz条件——简单说就是"输入变一点点，输出也只变一点点"——那实用性损失和扰动幅度成正比。mLDP的Property 1和2保证了扰动本身就小，所以损失可控。
...
- 直接删掉所有敏感token
- 隐私最强，但实用性几乎归零——LLM看到"My age is [REDACTED], suggest retirement plan"无法回答
...
Redaction是最简单的——删掉就行。隐私完美，但实用性崩溃。你让LLM帮你做退休规划，把年龄删了，它怎么给建议？
...
右边的表格是核心结果。看GPT-4o英→德的数据：Name属性，未净化是0.287，Prϵϵmpt是0.278，差距0.009，基本在统计波动范围内。而Papillon只有0.175，差距明显。

为什么Prϵϵmpt在翻译上表现这么好？翻译任务中，Name和Money用FPE处理，Age用mLDP处理。由于翻译结果本身不依赖敏感token的具体值，FPE加密后格式不变，mLDP的小扰动对翻译也几乎没有影响。
...
- 75th百分位的净化误差反而低于未净化基线——论文认为噪声可能使LLM避开了某些异常响应
...
- 隐私预算和误差之间有清晰的权衡曲线
- $\epsilon \geq 1.0$ 时，中位误差已经很小

<!--
第四组实验是最有挑战性的——多轮金融Q/A。

这个任务不像翻译那样对数值不敏感，LLM需要对财务数据做实际计算：利润率、增长率、比较大小等。如果mLDP加的噪声太大，计算结果就会偏离。
...
Prϵϵmpt是唯一全部打勾的方案，代价是只处理独立token级别的隐私

<!--
这张表是整篇论文的总结性对比。

横向看四个目标：无状态、形式化隐私保证、高实用性、资源高效。纵向看各种方案。

密码学方案（HE/MPC）在隐私和实用性上都没问题，但资源开销是致命弱点。各种DP方案有形式化保证，但实用性不行——要么噪声太大（文本级DP），要么改变了token类型（嵌入级DP）。替换方案和Papillon实用性可以，但没有形式化保证且有状态。

Prϵϵmpt的策略是：不试图保护所有类型的隐私，只聚焦于独立token级别的敏感信息，然后在这个范围内同时满足四个目标。这个范围限制是论文明确承认的——上下文语义层面的隐私（比如"这段话暗示用户有抑郁症"）留作future work。

我认为这个取舍是合理的。独立token的隐私风险是最直接、最容易被利用的——对手看到一个SSN就能造成实际损害，不需要理解上下文。先解决这个"低垂的果实"，是务实的选择。
-->
...
"不是所有token都需要同一种保护" — 这个观察让Prϵϵmpt同时满足了四个设计目标

### 值得思考的问题

Prϵϵmpt只保护独立token级别的隐私。但实验中80%的长文本prompt通过上下文语义就泄露了角色身份——这恰好是Prϵϵmpt不保护的部分。在什么场景下，token级别的保护已经足够？在什么场景下，它是不够的？
...
我觉得这篇论文最值得记住的判断是：不是所有敏感token都需要同一种保护方式。格式决定一切的token用FPE，数值有意义的token用mLDP——这个分类让系统在四个设计目标上同时达标。
