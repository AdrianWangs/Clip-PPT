...
<!--
现在我们来看一个可选的优化模块：方差引导式降维。坦白说，这个模块一开始只是为了减少内存和通信开销——CLIP特征是512维，我们想通过简单的降维压缩来减少存储成本、传输时间和计算开销。但实验结果却给了我们一个完全意料之外的惊喜。

我们的最初想法很朴素：CLIP特征虽然是512维，但并非所有维度都同等重要。低方差的维度在整个数据集上变化很小，鉴别能力可能比较弱，删除它们应该不会有太大影响，还能节省资源。

为什么这样做有理论依据呢？因为CLIP是在海量数据上预训练的通用模型，它的512个维度要覆盖各种场景——动物、植物、建筑、交通工具等等。但在实际应用中，我们的数据集往往只聚焦在某个特定领域。比如，CLIP的某个维度可能主要用来区分动植物特征，但如果我们的数据集主要是工业用品、电子产品这类图像，那这个维度在我们的数据集上就几乎用不上，所有样本在这个维度上的取值都很接近，方差就会很小。去掉这些对当前数据集不相关的维度，既能压缩开销，理论上也不应该损失太多精度。

技术实现也很直接。我们计算每个维度在整个数据集上的方差，用这个公式：σ²_j等于所有样本在第j维的取值减去均值的平方和，除以样本数n。然后选择方差最小的k个维度删除，这里k等于总维度数d乘以压缩比例r。比如d等于512、r等于5%时，就删除方差最小的约26维，节省5%的开销。

关键问题是：这个删除规则必须在客户端和服务器之间保持一致，同时不能泄露具体哪些维度被删除了。我们的解决方案是用AES加密这个压缩规则。数据所有者计算出要删除的维度集合，用密钥加密后，同时给客户端和服务器。这样双方能用同样的规则压缩向量，但攻击者无法知道具体删除了哪些维度。

我们本来只是想验证一下压缩后准确率不要掉太多就行。但看右边的实验结果时，我们非常惊讶。这是在CIFAR-100上、beam size固定为10的情况下测试的。横轴是压缩比例，纵轴是Recall@1准确率。曲线的走势完全出乎我们的意料。

当我们不压缩时，准确率是90%，这是基线。按预期，压缩应该会让准确率下降。但当压缩2.5%的维度时，准确率不降反升到92%！当压缩5%时，准确率达到峰值93%，比基线还高了3个百分点！这个结果非常违反直觉——我们原本只是想降低成本，没想到准确率反而提升了。

但随着压缩比例继续增加，准确率开始下降。7.5%压缩时回到90%，和基线持平。10%压缩时降到88%，比基线低2个点。15%压缩时继续下滑。这说明压缩过度后，我们开始删除有用的信号维度了。曲线呈现出一个非单调的形状，这在一开始完全没有预料到。

发现这个现象后，我们仔细分析了原因。可以用信号-噪声分解来理解。把归一化的特征向量看成真实信号s加上噪声η。余弦相似度可以近似为信号内积加噪声内积，除以包含信号和噪声范数的分母。

原来，当我们去除低方差维度时，主要删除的是噪声维度。这有两个效果：第一，减少了分母中的噪声范数项，让分母变小；第二，减少了分子中噪声内积的方差，让信号内积占主导。两者共同作用，提升了信噪比。

信噪比提高后，聚类质量会变好。在构建球形树时，如果特征噪声少，球面k-Means算出的簇中心就更稳定，更能代表真实的语义中心。搜索时，查询向量与这些稳定的球心计算相似度，就不容易走错方向，准确率自然提升。这解释了为什么删除维度反而提升了准确率。

但如果压缩过度，比如删除10%以上的维度，我们就开始丢失有用的信号维度了。这时信号内积本身变小，准确率自然下降。所以5%压缩是一个最优点，完美平衡了噪声消除和信号保留。

这个意外发现的实践意义非常大：我们不仅减少了约26维的存储和通信开销，节省约5%的资源，同时还意外地把准确率从90%提升到93%。这是一个完全没有预期到的双赢结果。在后续的实验中，我们默认使用5%的压缩比例。
-->
...
数据所有者这边，首先用CLIP提取所有图片的特征并L2归一化，得到单位球面上的向量。然后用球面k-Means递归构建树索引，k=10，深度3-4层。接着用ASPE方案加密所有节点向量，保持内积关系。同时构建Merkle树，为每个节点生成哈希。最后公开根哈希，把加密树上传到云端。

数据用户查询时，本地提取CLIP特征并加密，生成陷阱门发给云端。云端执行束搜索，beam size一般是3到10，逐层扩展保留top-β节点，最后返回top-k文档和验证证明。
