...
CLIP特征并非所有维度都同等重要，低方差维度可能主要携带噪声。

**技术方案：**
- 计算每个维度的方差：$\sigma^2_j = \frac{1}{n}\sum_{i=1}^n (x_{ij} - \bar{x}_j)^2$
- 选择方差最小的 $k = d \times r$ 个维度删除（$r$ 为压缩比例）
- 用AES加密压缩规则，确保客户端和服务器同步

**理论直觉：信号-噪声分解**

低方差维度鉴别能力弱 → 去除后提升信噪比 → 聚类质量改善 → 球心更稳定 → 搜索更准确

**意外发现：** 适度压缩不仅节省资源，还能提升准确率！
...
<!--
现在我们来看一个可选但非常有效的优化模块：方差引导式降维。这个模块的设计初衷是为了减少内存和通信开销，但实验结果却给了我们一个意外的惊喜——它不仅减少了开销，还提升了检索准确率。

首先看核心思想。CLIP特征虽然是512维，但并非所有维度都同等重要。我们的假设是：低方差的维度，在整个数据集上变化很小，鉴别能力弱，可能主要携带的是噪声信息。

技术实现很直接。我们计算每个维度在整个数据集上的方差，用这个公式：σ²_j等于所有样本在第j维的取值减去均值的平方和，除以样本数n。然后选择方差最小的k个维度删除，这里k等于总维度数d乘以压缩比例r。比如d等于512、r等于5%时，就删除方差最小的约26维。
...
现在看右边的实验结果，这是在CIFAR-100上、beam size固定为10的情况下测试的。横轴是压缩比例，纵轴是Recall@1准确率。曲线非常有意思。

当我们不压缩时，准确率是90%，这是基线。当压缩2.5%的维度时，准确率不降反升到92%。当压缩5%时，准确率达到峰值93%，比基线还高了3个百分点！这个结果非常违反直觉——我们删除了维度，准确率反而提升了。

但随着压缩比例继续增加，准确率开始下降。7.5%压缩时回到90%，和基线持平。10%压缩时降到88%，比基线低2个点。15%压缩时继续下滑。这说明压缩过度后，我们开始删除有用的信号维度了。

为什么会出现这个非单调的曲线？我们可以用信号-噪声分解来理解。把归一化的特征向量看成真实信号s加上噪声η。余弦相似度可以近似为信号内积加噪声内积，除以包含信号和噪声范数的分母。

当我们去除低方差维度时，主要删除的是噪声维度。这有两个效果：第一，减少了分母中的噪声范数项，让分母变小；第二，减少了分子中噪声内积的方差，让信号内积占主导。两者共同作用，提升了信噪比。

信噪比提高后，有什么好处呢？聚类质量会变好。在构建球形树时，如果特征噪声少，球面k-Means算出的簇中心就更稳定，更能代表真实的语义中心。搜索时，查询向量与这些稳定的球心计算相似度，就不容易走错方向，准确率自然提升。

但如果压缩过度，比如删除10%以上的维度，我们就开始丢失有用的信号维度了。这时信号内积本身变小，准确率自然下降。所以5%压缩是一个最优点，完美平衡了噪声消除和信号保留。

这个发现的实践意义在于：我们不仅减少了约26维的存储和通信开销，节省约5%的资源，同时还把准确率从90%提升到93%。这是一个双赢的结果。在后续的实验中，我们默认使用5%的压缩比例。
-->
