...
<!--
大家好，今天我要分享的论文是一篇机器学习比较经典的一篇论文，全称是“从自然语言监督中学习可迁移的视觉模型”。这是OpenAI在2021年发表在ICML上的一篇论文，这篇论文在RAG，也就是向量检索数据库领域是一个里程碑级别的论文。
-->
...
<!--
在正式开始介绍论文之前，我想先花一点时间解释一下，为什么我的研究方向是可搜索加密，但是会来分享一篇AI领域的顶会论文。

首先就是，考虑到其实我们这一届只有我做可搜索加密，所以主要面向研一的同学去做分享，然后第一次讲我就先选择一个比较简单的论文去讲，然后结合一下可搜索，我觉得这样会比较容易方便研一的同学大致去了解这个方向。

然后就是一些具体的大方向方面的原因，我的主要研究方向是可搜索加密，简单来说，就是如何在加密的数据上进行有效的搜索。在看论文的过程中，我发现绝大多数可搜索加密方案，其底层的数学原理都依赖于向量内积来计算相关性。而今天我们要讲的CLIP模型，它最后生成的结果叫embedding嵌入向量，本质上也是向量。这就让我产生了一个想法：我们是不是可以直接把CLIP生成的这些强大的多模态向量，无缝对接到现有的可搜索加密框架中呢？如果可以，那就意味着我们能够在保证数据隐私安全的前提下，实现过去传统可搜索加密很难做到的多模态搜索和语义搜索。这就理论上可以实现上学期沈老师开学讲的，基于语义的搜索。这就是我今天分享这篇论文的初衷。

然后针对新同学，我在最后也会简单地讲解一下，这个论文得到的模型，如何与可搜索加密结合起来，实现跨模态可搜索加密

然后就是，其实目前跨模态可搜索方案2025年已经出现了多篇论文，其中我看到的有一篇就是结合的Clip模型，所以我觉得未来这个方向也是比较容易出成果的
-->
...
<!--
基于刚才的背景，这篇论文提出了三个核心问题。第一，我们能否也用自然语言，而不是固定的标签，来指导视觉模型的训练？第二，我们能不能让模型实现真正的零样本识别，也就是即使没有见过某个物体的标签，也能认出或者猜出这个物体的标签，或者说是名字？第三，我们能否构建一个统一的多模态模型，而不是为每个视觉任务都单独训练一个模型？CLIP的目标，就是回答这些问题，让计算机能像人一样，通过语言来理解视觉世界。
-->
...
为了让大家更直观地理解这个区别，我们来举一个简单的例子：识别一张狗的图片。
...
<div class="f几lex justify-center mt-1">
  <img src="./image/slides/29b20c8616a19cf02e21d1dfcad2cad2fa7e0b25aa51329ec499cd8a5ae70c32.jpg" alt="Figure 1: Summary of CLIP's approach" class="w-10/14">
</div>
...
这个向量的具体形式和维度是我们网络架构设计的一部分,也就是是我们人为基于经验设计出来的，但它的具体内容——也就是每个维度代表什么语义概念——是通过对比学习训练出来的。训练过程中，模型学会了把语义相似的图像和文本映射到嵌入空间中相近的位置，这就是CLIP能够实现zero-shot能力的核心机制。
-->
...
<div class="grid grid-cols-2 gap-6 mt-4">
  ...
  <div class="bg-green-50 p-4 rounded-lg">
    ...
    <p class="font-semibold mt-3">处理流程:</p>
    <p class="text-sm mt-1">
      对输入文本进行分词和编码，利用Transformer模型提取代表全局语义的 <code>[EOS]</code> 特征，同样经过线性投影和归一化，生成512维的文本嵌入向量T_e。
    </p>
  </div>
...
> BPE: 就是把高频出现的词汇尽可能合并成一个token

> 为什么这个EOS能够表示全局语义？
> 
> 准确来说，是表示EOS的向量能够表示整句话的意思，因为Transformer的架构其实就是为了把每个词语的向量给进行一个细微地调整，使得其能够具有在这个语境中的额外的意思，所以EOS其实是经过调整后的向量，自然可以表示整个语句的意思
-->
...
先统一一下符号。图像我们叫 I1 到 IN，文本叫 T1 到 TN。两者之间的相似度分数我们记成 $s_{ij} = I_i * T_j$,跟图里面保持一致，这个其实就等于右边公式里的 logits。本质上就是余弦相似度，然后除以一个温度参数 τ。
...
![Figure 2: Training Efficiency Comparison](./image/slides/2a720f1bf0ffbb8263f721a05c61159b14512e05d24cc9c35d15ff9222dd50ba.jpg)
...
2.  **相似度计算**：
    计算批次内所有图文对的**余弦相似度**，形成 $logits$ 矩阵，并由可学习的温度 $\tau$ 进行缩放。
...
## CLIP核心创新点
::left::
- **范式创新**：首次用自然语言作为监督信号，打破传统固定标签限制
- **架构创新**：双编码器+对比学习，构建统一图文语义空间
- **能力创新**：实现开放词汇的零样本学习，泛化能力突破
- **效率创新**：对比学习比生成式任务高效10倍以上

::right::
<img src="./image/slides/clip.png" class="w-10/12">

<!-- 备注：从范式、架构、能力、效率四个维度总结CLIP的核心创新，突出其突破传统视觉模型的关键贡献 -->
...
- **数学统一**：通过归一化处理，点积运算等价于余弦相似度
- **向量替换**：将归一化后的CLIP embedding应用于基于点积的加密方案
- **语义搜索**：在加密数据上实现基于自然语言的多模态搜索
- **隐私保护**：在不泄露数据隐私的前提下实现智能语义搜索
</div>
...
Encrypt函数负责加密数据以及其索引，先用CLIP将图像或文本编码成向量，这个向量当做数据的索引，归一化后用矩阵M加密。

然后数据随便用什么方法加密都行，可搜索加密的核心不在这一块

Trapdoor函数为查询关键字（这个方案里面也可以是一个图片）生成搜索陷门，其实很简单，就是也有Clip将查询进行编码成向量，然后用M的逆转置对查询向量进行变换。

Search函数在遍历所有的加密索引，与Trapdoor做内积计算相似度，最后Trapdoor和加密索引的内积正好等于明文向量的内积。然后我们就可以不解密的情况下获取到原始数据的相似度

当然了，这只是一个简单的例子，这里介绍的ASPE也是一个安全性级别最低的ASPE，后面还有一些什么向量分裂、随机填充噪声这种提升安全性的方法，会在很多论文里面都可以看到
-->
