...
我们选了k'=160，r=0.03这个配置，此时攻击效果大概是30，算是中等程度的隐私保护。

对比一下三个方案的表现：
- 隐私忽略方案最快，3.15毫秒加8KB，但完全不安全。
- 完全隐私方案要2.72小时和1.43GB，这在实际应用中根本不可行。
- RemoteRAG直接模式只需要0.67秒和46.66KB，OT模式是0.68秒和108.24KB。

看开销分解，主要的计算量确实集中在模块2(a)的同态加密部分，占了95%以上。但关键是我们把计算范围缩小到了k'=160个文档，而不是对全部N个文档做加密计算，所以总体速度还是很可以接受的。

这就是RemoteRAG的实用性所在——在保护隐私的同时，还能保持实际可用的效率。
-->
